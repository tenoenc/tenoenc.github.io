...
본 회고는 로컬 네트워크 패킷 분석 시스템 개발 중 발생한 데이터 처리 지연 현상을 분석하고, 이를 아키텍처 관점에서 해결한 과정을 기술한다. 특히 대용량 트래픽(Elephant Flow) 환경에서 발생한 **Redis N+1 입출력 문제**를 진단하고, **Pipelining 및 Bulk 연산**을 도입하여 시스템 처리량(Throughput)을 약 700% 향상시킨 경험을 중점적으로 다룬다.
...
* **증상**: 시스템 가동 20분 경과 시점에서 약 8분의 데이터 처리 지연(Lag) 발생.
...
* **구조적 결함**: `PacketService`가 Kafka로부터 수신한 패킷 리스트를 순회(Iterate)하며, 개별 패킷마다 Redis에 동기(Synchronous) 요청을 보내고 있었다.
* **N+1 문제**: 패킷 1개를 분석하기 위해 `위협 탐지(3회)` + `세션 조회(1회)` + `세션 갱신(1회)` 등 평균 5회의 Redis 네트워크 호출이 발생했다.
* 배치 사이즈가 3,000개일 경우, 단일 트랜잭션 내에서 **15,000회의 네트워크 RTT(Round Trip Time)**가 발생.
* 로컬 환경(Localhost)임에도 불구하고, I/O Wait 시간이 CPU 연산 시간을 압도하며 전체 처리 속도를 저하시킴.



### Solution Strategy

네트워크 I/O 비용을 최소화하기 위해 단건 처리 방식을 **일괄 처리(Bulk Operation)** 방식으로 리팩토링했다.
...
서비스 계층에서는 DB 접근 로직을 루프 밖으로 끄집어내어(Hoisting), **"Bulk Read → In-Memory Calculation → Bulk Write"** 패턴으로 재구성했다.
...
2. **배치 처리의 중요성**: 스트림 데이터 처리(Stream Processing)라 하더라도, 내부 로직은 가능한 한 마이크로 배치(Micro-batch) 단위로 그룹화하여 처리하는 것이 효율적이다.
3. **TDD의 가치**: 구조를 대대적으로 변경하는 리팩토링 과정에서, 사전에 작성된 단위 테스트(Unit Test)가 변경에 따른 사이드 이펙트를 감지하고 로직의 정합성을 유지하는 역할을 수행했다.
